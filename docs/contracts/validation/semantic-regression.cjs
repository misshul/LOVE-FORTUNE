...
function guard(input){const f=structuredClone(input).sort((x,y)=>x.id.localeCompare(y.id)),W0=sum(f.map(x=>x.w));let points=0;for(const x of f){const impact=W0?50*x.w*Math.abs(x.v)/W0:0;points+=Math.sign(x.v)*Math.min(impact,20);if(impact>20)x.w=.4*W0/Math.abs(x.v);}return{status:'PASS',f,raw:50+points};}
for(const t of v.guardrail){let r=guard(t.features);eq(r.status,t.expected);if(t.raw!==undefined)eq(r.raw,t.raw);eq(r,guard([...t.features].reverse()));old++;}
