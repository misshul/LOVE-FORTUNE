...
If usableEvidence is empty: categoryScore=null, resultConfidence=0, status=INSUFFICIENT_DATA; retain computed coverage. Exclude insufficient categories from overall aggregation; all insufficient gives null overall. Do not use ambiguous E=0 as a coverage reset. SC-07 v2 never forces positive usable weight to zero and does not alter structural coverage.

With usable evidence, categoryResultConfidence=sum(available preConfidenceWeight*featureConfidence)/sum(available preConfidenceWeight); the category wire resultConfidence=coverage*categoryResultConfidence. No available/usable evidence gives confidence0. overallPreCoverageConfidence is the SC-01 categoryWeight-weighted mean of pre-coverage category confidence over computable categories. overallWireConfidence=overallCoverage*overallPreCoverageConfidence; resultConfidence carries this wire value. Source/category M2 does not change that computable-category denominator. COMMUNICATION is excluded from all overall denominators; coverage eligible weight=.86. Verified means .691385 pre-coverage and .573655 wire are reference statistics only.

Daily source confidence uses exactly four nominal samples06:00/12:00/18:00/23:00. Each sample confidence is in[0,1]; unavailable samples contribute0. sourceDailyConfidence=sum(all four sampleConfidence)/4. Signal aggregation still excludes unavailable samples. V1 has only SAJU daily confidence; Zodiac is static context, not a Daily source. No Saju sample gives0. Weekly/Monthly confidence is the arithmetic mean of valid Daily confidence; Yearly is the mean of valid Monthly confidence; empty inputs give0. Missing lifetime retains INSUFFICIENT_DATA/confidence0.

---

# SC-02 Rule catalog and candidates

Saju has14 family records,9 enabled scoring families with16 variants/37 decimal mappings and5 context-only families. See saju-catalog-v1.md. Astrology109/228 remains unchanged in ADVANCED / DEFERRED. Zodiac V1 has20 numeric mappings with fixed confidence3/4 and NO_SC07. Mapping signs match decimal signedValues. The preserved Daily inventory contains134 enabled rules/285 mappings, but V1 scope selects only SAJU9/19; Astrology125/266 is Advanced deferred. Catalog blockers NONE. Daily INTERNAL sample identity is defined in daily-catalog-v1.md; Lifetime/public identity remains unchanged.
...
# SC-03 Outer cap (ADVANCED / DEFERRED)
...
V1 SAJU metadata allowlist: pillar,element,relation,candidateCount,availabilityRatio,sampleCount,referenceId,ruleVariant,candidates,transformationStatus. candidates is a scalar array and non-identity; transformationStatus=UNASSESSED is non-identity and STEM_COMBINATION-only. SAJU identity subset: pillar,relation,referenceId,ruleVariant. Zodiac metadata is signA,signB,pairId,relation,modelVersion,dateRangeVersion, all identity-relevant. Unknown fields fail closed. Planet/aspect/orb/orbCloseness metadata belongs only to archived Advanced schemas.
...
Selection maxima: Weekly best/caution2 each, Monthly5 each, Yearly3 each. Best score DESC, caution score ASC, then resultConfidence DESC and chronological ASC. Compare exact rational/integer internal scores before API rounding, with integer cross multiplication; never binary-double equality or epsilon. Monthly means full-precision valid Daily scores; Yearly means full-precision valid Monthly scores; volatility is population standard deviation. Round only API scores to four decimals HALF_UP; status uses the canonical score before display formatting.

---

# SC-07 Daily category, Actions and guardrail

Category daily signal uses category-specific Saju evidence. Internal COMMUNICATION mappings remain, but its Lifetime baseline and final Daily category score are null. categoryDailyDelta=18*categoryDailySignal; categoryDailyScore=clamp(lifetimeCategoryScore+categoryDailyDelta,0,100). No computable category period data gives delta0 and periodStatus=INSUFFICIENT_PERIOD_DATA. Do not copy overall signal into categories.
...
Current V1 Daily authority: Zodiac V1 scope selects SAJU9/19, K=18, exact signed-peak ties. Daily SC-07 remains NOT_APPLIED. Source-internal M1 eligibility remains; old dual-source .50/.50 is Advanced-only. Public V1 schemas and context are updated; privacy is unchanged.
