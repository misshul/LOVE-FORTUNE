...
[Astrology rule schema](schemas/astrology-rule.schema.json) owns planetA,planetB,aspect, pairWeightSource and decimal direction mapping. additionalProperties=false at rule and mapping levels. rule.schema.json continues to validate legacy/Saju inventory; its MIXED and unit-sign contracts are not applied to these new Astrology rules. Every enabled v1 rule has mappings, requiresBirthTime=false,contextOnly=false,ruleWeight=1,version=1.0.0. A rule's exactAngle/baseWeight/baseOrb resolves from the catalog aspectDefinitions by aspect. Planet weights, pair overrides and orb adjustments resolve from referenceTables. The validator checks these joins and the exact approved mapping matrix; no free-form metadata stores hidden rule definitions.
...
Contract Freeze PASS. Astrology Catalog APPLIED and Astrology Catalog Readiness READY. Overall Score Engine BLOCKED_CATALOG: SAJU_RULE_CATALOG_APPROVAL and DAILY_RULE_CATALOG_APPROVAL. Production BLOCKED_EXTERNAL: SAJU_DAY_PILLAR_EPOCH and EPHEMERIS_PROVIDER. SCORING_RULE_CATALOG_APPROVAL remains the historical umbrella; these are its remaining components. Catalog readiness does not mean production engine implementation or provider validation.
